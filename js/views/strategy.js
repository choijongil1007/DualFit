...
export function renderStrategy(container, dealId, isTab = false) {
    ...
    if (!isTab) {
        ...
        document.getElementById('btn-recalc-strategy').addEventListener('click', () => {
             generateStrategyAI(deal, bizScore, techScore, lowItems);
        });
    } else {
        const recalcBtn = document.getElementById('btn-recalc-tab-strategy');
        if (recalcBtn) {
...
function renderAIContent(result) {
    const container = document.getElementById('strategy-ai-content');
    if (!container) return;
...
async function generateStrategyAI(deal, bizScore, techScore, lowItems) {
    const container = document.getElementById('strategy-ai-content');
