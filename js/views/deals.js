...
export function renderDeals(container) {
    ...
    const html = `
        <div class="flex justify-between items-center mb-8">
            ...
            <button id="btn-create-deal" class="bg-black text-white px-4 py-2 rounded hover:bg-zinc-800 transition text-sm font-medium shadow-sm">
        ...
        <div id="create-modal" class="fixed inset-0 z-[100] hidden flex items-center justify-center p-4">
            ...
            <div class="relative w-full max-w-lg bg-zinc-900 text-white border border-white/10 rounded-xl shadow-2xl p-6 sm:p-8 animate-modal-in">
                ...
                <form id="create-form" class="space-y-4">
                    <div>
                        ...
                        <input type="text" name="clientName" required class="w-full bg-black/50 border border-transparent focus:border-white/30 rounded-lg p-2.5 text-sm text-white placeholder-gray-600 outline-none transition-all" placeholder="e.g. Acme Corp">
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-gray-400 uppercase mb-1.5">Project Name</label>
                        <input type="text" name="dealName" required class="w-full bg-black/50 border border-transparent focus:border-white/30 rounded-lg p-2.5 text-sm text-white placeholder-gray-600 outline-none transition-all" placeholder="e.g. Cloud Migration Phase 1">
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-xs font-semibold text-gray-400 uppercase mb-1.5">Client Contact</label>
                            <input type="text" name="clientContact" class="w-full bg-black/50 border border-transparent focus:border-white/30 rounded-lg p-2.5 text-sm text-white outline-none transition-all">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-gray-400 uppercase mb-1.5">Our Contact</label>
                            <input type="text" name="internalContact" class="w-full bg-black/50 border border-transparent focus:border-white/30 rounded-lg p-2.5 text-sm text-white outline-none transition-all">
                        </div>
                    </div>
                    <div>
                         <label class="block text-xs font-semibold text-gray-400 uppercase mb-1.5">Solution</label>
                         <input type="text" name="solution" class="w-full bg-black/50 border border-transparent focus:border-white/30 rounded-lg p-2.5 text-sm text-white outline-none transition-all">
                    </div>
                    <div>
                         <label class="block text-xs font-semibold text-gray-400 uppercase mb-1.5">Target Date</label>
                         <input type="date" name="purchaseDate" class="w-full bg-black/50 border border-transparent focus:border-white/30 rounded-lg p-2.5 text-sm text-white outline-none transition-all [color-scheme:dark]">
                    </div>
                    <div>
                         <label class="block text-xs font-semibold text-gray-400 uppercase mb-1.5">Memo</label>
                         <textarea name="memo" class="w-full bg-black/50 border border-transparent focus:border-white/30 rounded-lg p-2.5 text-sm text-white outline-none transition-all resize-none" rows="3"></textarea>
                    </div>
        ...
        <div id="delete-modal" class="fixed inset-0 z-[110] hidden flex items-center justify-center p-4">
            ...
            <div class="relative w-full max-w-sm bg-zinc-900 text-white border border-white/10 rounded-xl shadow-2xl p-6 animate-modal-in text-center">
                ...
                <div class="flex gap-3 justify-center">
                    <button type="button" class="btn-close-delete-modal px-4 py-2 bg-zinc-800 hover:bg-zinc-700 text-white rounded-lg text-sm font-medium transition-colors">Cancel</button>
    ...
    return `
        <div class="group relative bg-white border border-gray-200 rounded-lg p-5 hover:shadow-md transition-all cursor-pointer deal-card" data-id="${deal.id}">
            <div class="flex justify-between items-start mb-3">
                ...
                <button type="button" class="btn-delete-deal text-gray-300 hover:text-red-500 p-1.5 rounded hover:bg-red-50 opacity-0 group-hover:opacity-100 transition-all z-10" data-id="${deal.id}" title="Delete Deal">
